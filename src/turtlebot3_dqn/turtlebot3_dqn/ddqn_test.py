...
class DDQNTest(Node):
    ...
    def __init__(self, stage, load_episode):
        ...
        self.model_path = os.path.join(self.model_dir_path,
            'stage'+str(self.stage)+'_episode'+str(self.load_episode)+'_ddqn'+'.h5'
        )
        ...
        if self.load_model:
            ...
            with open(os.path.join(
                    self.model_dir_path,
                    'stage'+str(self.stage)+'_episode'+str(self.load_episode)+'_ddqn'+'.json')) as outfile:
    ...
    def train_model(self, target_train_start=False):
        ...
        x_batch = numpy.empty((0, self.state_size), dtype=numpy.float32)
        y_batch = numpy.empty((0, self.action_size), dtype=numpy.float32)
...
def main(args=None):
    ...
    dqn_test = DDQNTest(stage, load_episode)
    try:
        while rclpy.ok():
