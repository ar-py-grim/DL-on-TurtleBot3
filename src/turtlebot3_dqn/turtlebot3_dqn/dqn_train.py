    ...
    def __init__(self, stage_num, max_training_episodes):
        ...
        if self.load_model:
            ...
            with open(os.path.join(
                ...
                self.step_counter = param.get('step_counter',0)

        if LOGGING:
            tensorboard_file_name = current_time+'_dqn_stage'+str(self.stage)+'_load_episode'+str(self.load_episode)+'_state'+str(self.state_size)
            dqn_reward_log_dir = 'dqn_logs/gradient_tape_dqn/' + tensorboard_file_name
...
def main(args=None):
    ...
    max_training_episodes = args[2] if len(args) > 2 else '10001'
